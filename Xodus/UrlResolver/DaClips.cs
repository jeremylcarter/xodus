...
{
    public class DaClips : IResolver
    {
        private readonly string url;

        public DaClips(string uri)
        {
            url = uri;
            SourceName = "DACLIPS";
        }
        ...
        {
            Debug.WriteLine("DaClips");
            var file = "";

            try
            {
                var mediaid = url.Substring(url.LastIndexOf("/", StringComparison.OrdinalIgnoreCase) + 1);
                var newUrl = $"http://daclips.in/{mediaid}";
