...
namespace CloudFlareUtilities
{
    ...
    public class ClearanceHandler : DelegatingHandler
    {
        ...
        /// <summary>
        ///     Creates a new instance of the <see cref="ClearanceHandler" /> class with a <see cref="HttpClientHandler" /> as
        ///     inner handler.
        /// </summary>
        ...
        /// <summary>
        ///     Releases the unmanaged resources used by the <see cref="ClearanceHandler" />, and optionally disposes of the
        ///     managed resources.
        /// </summary>
        ...
        {
            var IdCookieBefore = ClientHandler.CookieContainer.GetCookiesByName(request.RequestUri, IdCookieName)
                .FirstOrDefault();
            var clearanceCookieBefore = ClientHandler.CookieContainer
                .GetCookiesByName(request.RequestUri, ClearanceCookieName).FirstOrDefault();
            ...
            var IdCookieAfter = ClientHandler.CookieContainer.GetCookiesByName(request.RequestUri, IdCookieName)
                .FirstOrDefault();
            var clearanceCookieAfter = ClientHandler.CookieContainer
                .GetCookiesByName(request.RequestUri, ClearanceCookieName).FirstOrDefault();
        ...
        private static bool IsClearanceRequired(HttpResponseMessage response)
        {
            ...
            var isCloudFlareServer =
                response.Headers.Server.Any(i => i.Product != null && i.Product.Name == CloudFlareServerName);
